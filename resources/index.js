...
function main() {
    const date = new Date();
    let dateEvent;
    if (eventDateTemp.getTime() < date.getTime() || recalc) {
        recalc = false;
...
function getScheduleTimes(schedule, currentTime) {
    if (schedule.specialDays.length > 0) {
        for (let i = 0; i < schedule.specialDays.length; i++) {
...
function updateMenu(school, schedule) {
    ...
    document.getElementById("settingsIcon").onclick = function (e) {
        ...
        bootbox.dialog({
            ...
            closeButton: true,
            onEscape: function () {
                localStorage.setItem("school", schoolSelect);
                localStorage.setItem("schedule", scheduleSelect);
...
window.onload = function() {
    ...
    $.getJSON('./resources/timestamps.json', function (data) {
        ...
        schoolSelect = localStorage.getItem("school");
        if (!schoolSelect) schoolSelect = timestampsJSON.schools[0].id;

        scheduleSelect = localStorage.getItem("schedule");
        if (!scheduleSelect) scheduleSelect = timestampsJSON.schools[0].schedules[0].id;
